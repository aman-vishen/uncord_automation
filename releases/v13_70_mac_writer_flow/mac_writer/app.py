...
APP_VERSION = "13.70-ETE-MAC-WRITER-FW-FIRST-FIRSTBOOT-FACTORY-WRITE-FIRSTBOOT-VERIFY"
...
def parse_commands_file(path: Path) -> tuple[list[str], list[str], list[str]]:
    ...
    sections = {"FIRMWARE": [], "POST_FIRMWARE": [], "WRITE": [], "POST_WRITE": [], "VERIFY": [], "FINALIZE": []}
    current = None
    for line_no, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        line = raw.strip()
        if not line or line.startswith("#") or line.startswith(";"):
...
async def _open_logged_in_telnet(router: RouterTarget, emit):
...
async def run_single_telnet_command(router: RouterTarget, command: str, emit, *, reboot_expected: bool = False) -> None:
    command=(command or "").strip()
    if not command:
        return
    reader, writer = await _open_logged_in_telnet(router, emit)
    try:
        emit(f"> {command}")
        writer.write(command + "\r\n")
        await asyncio.sleep(max(0.3, router.command_delay))
        if reboot_expected:
            try:
                ...
                # firstboot/reboot is expected to tear the session down.
                pass
        else:
            ...
            if any(token in upper for token in ("UNKNOWN COMMAND", "NOT FOUND", "INVALID", "FAILED", "FAIL", "ERROR")):
                raise AppError(f"Command failed: {command} | {response.strip()}")
    finally:
        try:
            writer.close()
        except Exception:
...
async def verify_identity_only(router: RouterTarget, mac: str, serial: str, gpon: str, seq: int,
    ...
    try:
        ...
        emit("RESULT: PASS - identifiers verified after reboot." if passed else "RESULT: FAIL - one or more identifiers did not match after reboot.")
...
class ClientApp(tk.Tk):
    ...
    def _cycle_worker(self, router: RouterTarget, client: ServerClient, request_id: str, cfg, write_cmds, verify_cmds, finalize_cmds, selected_identity=None):
        ...
        try:
            ...
            # ------------------------------------------------------------
            # STEP 2: FIRSTBOOT AFTER FIRMWARE
            # ------------------------------------------------------------
            firstboot_pre = cfg.get("POST_FIRMWARE_FIRSTBOOT_COMMAND", "firstboot -y -r").strip()
            emit("--- FIRSTBOOT AFTER FIRMWARE ---")
            asyncio.run(run_single_telnet_command(router, firstboot_pre, emit, reboot_expected=True))
            wait_for_router_after_firmware(
                router,
                float(cfg.get("POST_FIRMWARE_FIRSTBOOT_INITIAL_DELAY_SECONDS", "10")),
            ...
            # ------------------------------------------------------------
            # STEP 3: FACTORY MODE AFTER FIRSTBOOT
            # ------------------------------------------------------------
            factory_cmd = cfg.get("POST_FIRSTBOOT_FACTORYMODE_COMMAND", "prolinecmd factorymode set 1").strip()
            emit("--- FACTORY MODE AFTER FIRSTBOOT ---")
            asyncio.run(run_single_telnet_command(router, factory_cmd, emit, reboot_expected=False))
            emit("Factory mode command completed.")

            # ------------------------------------------------------------
            # STEP 4: RESERVE IDENTITY ONLY AFTER ALL PREPARATION SUCCEEDS
            # ------------------------------------------------------------
            ...
            emit("Firmware + firstboot + factory mode complete. Reserving identity.")
            try:
                if selected_identity:
                ...
                if "NO PCB SERIAL NUMBER PRESENT" in str(exc).upper():
                    ...
                    self.events.put((
                        ...
                        (
                            router.key,
                            "NO PCB SERIAL NUMBER PRESENT — PCB must be linked before MAC Write. "
            ...
            set_pending_job(router.key, {
                ...
                "gpon_number": gpon,
                "seq": seq,
                "state": "RESERVED_AFTER_FW_FIRSTBOOT_FACTORYMODE",
                "created_at": time.time(),
            })
            ...
            # ------------------------------------------------------------
            # STEP 6: FIRSTBOOT AFTER IDENTITY WRITE
            # ------------------------------------------------------------
            post_write_firstboot = cfg.get("POST_WRITE_FIRSTBOOT_COMMAND", "firstboot -y -r").strip()
            emit("--- FIRSTBOOT AFTER MAC / SERIAL / GPON WRITE ---")
            asyncio.run(run_single_telnet_command(router, post_write_firstboot, emit, reboot_expected=True))
            wait_for_router_after_firmware(
                router,
                float(cfg.get("POST_WRITE_FIRSTBOOT_INITIAL_DELAY_SECONDS", "10")),
            ...
            # ------------------------------------------------------------
            # STEP 7: VERIFY IDENTITY AFTER REBOOT
            # ------------------------------------------------------------
            passed, checks = asyncio.run(
                verify_identity_only(router, mac, serial, gpon, seq, verify_cmds, emit)
            )
            checks["FIRMWARE"] = bool(firmware_enabled) or not require_fw
            checks["PRE_FIRSTBOOT"] = True
            ...
            if passed:
                ...
                detail = (
                    ...
                    + " | firstboot after firmware | factorymode=1 | identity written"
                    + " | firstboot after identity write | MAC/SN/GPON verified after reboot"
                )
            else:
                status = "FAIL"
                detail = f"Post-reboot identifier verification failed: {checks}"
    ...
    def _resume_worker(self, router, client, pending, cfg, write_cmds, verify_cmds, finalize_cmds):
        ...
        try:
            emit("Recovery: identity was already reserved. Firmware/pre-firstboot/factorymode will NOT be repeated.")
            asyncio.run(write_identity_only(router, mac, serial, gpon, seq, write_cmds, emit))
            post_write_firstboot = cfg.get("POST_WRITE_FIRSTBOOT_COMMAND", "firstboot -y -r").strip()
            emit("--- RECOVERY FIRSTBOOT AFTER IDENTITY WRITE ---")
            asyncio.run(run_single_telnet_command(router, post_write_firstboot, emit, reboot_expected=True))
            wait_for_router_after_firmware(
                router,
                float(cfg.get("POST_WRITE_FIRSTBOOT_INITIAL_DELAY_SECONDS", "10")),
            ...
            detail = "Recovered reserved identity | write -> firstboot -> post-reboot verify" if passed else f"Recovered cycle verification failed: {checks}"
            report = client.report(router, reservation_id, status, detail, serial, gpon)
            set_pending_job(router.key, None)
            self.events.put(("done", (router.key, mac, serial, gpon, status, report)))
        except Exception as exc:
            ...
            try:
                ...
                self.events.put(("log", (router.key, f"Could not report recovery ERROR: {report_exc}")))
                self.events.put(("cycle_error", (router.key, mac, error_text)))
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    card.set_step("reserve", "✓")
                    card.set_step("write", "…")
                    card.set_state("PROGRAMMING", "Preparation complete; writing MAC/SN/GPON, then firstboot and post-reboot verification",
