...
APP_VERSION = "13.71-ETE-MAC-REWRITE-ADMIN-RETEST"
...
def _rewrite_mark_final_failed(client: ServerClient, router: RouterTarget, identity: dict, cfg: dict[str, str], emit) -> dict:
    """Create a new FAIL Final Verification record without changing mac_pool state."""
    part = (cfg.get("REWRITE_PART_NUMBER", "AC3000-X31") or "AC3000-X31").strip()
    request_id = "rewrite-" + str(uuid.uuid4())
    mac = identity.get("mac", "")
    serial = identity.get("serial_number", "")
    gpon = identity.get("gpon_number", "")
    emit("Marking Final Verification as FAIL so this identity must be verified again...")
    check = client.call("POST", "/api/verify/check", {
    ...
    if not verification_id:
        raise AppError("Server did not return verification_id for rewrite invalidation.")
    result = client.call("POST", "/api/verify/result", {
        "client_id": client._client_id(router),
        "verification_id": verification_id,
        "status": "FAIL",
        "serial_scan_result": "PASS",
        "wifi_calibration_result": "",
        "bob_calibration_result": "",
        "firmware_result": "",
        "firmware_version": "",
        "led_result": "",
        "reset_result": "",
        ...
        "detail": (
            ...
            "MAC REWRITE PASS — prior product state requires Final Verification rerun."
        ),
    })
    emit(f"Final Verification rework marker created: FAIL ({verification_id})")
    return result
        ...
        try:
            ...
            if not passed:
                ...
                self.events.put(("done", (
                    ...
                    {"stats": {}, "rewrite": True, "final_verification_marked_fail": False},
                )))
                return
            ...
            self.events.put(("done", (
                router.key, mac, serial, gpon, "PASS",
                {
                    "stats": {},
                    "rewrite": True,
                    "final_verification_marked_fail": True,
                    "verification_result": fv,
