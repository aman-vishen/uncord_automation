...
APP_VERSION = "13.73-ETE-MAC-REWRITE-PASS-IDENTITY-ALLOWED"
...
def run_direct_firmware_update(router: RouterTarget, firmware_path: Path, cfg: dict[str, str], emit) -> dict:
    """Run the supplied OpenWrt updater logic directly from the MAC Rewrite.
...
def _rewrite_mark_final_failed(client: ServerClient, router: RouterTarget, identity: dict, cfg: dict[str, str], emit) -> dict:
    """Authorize Final Verification rework without changing mac_pool PASS state.

    The central server blocks a normal repeat Final Verification after a PASS.
    A successful MAC Rewrite creates an explicit rework authorization instead.
    That authorization remains active through FAIL/ERROR retries and is consumed
    only when the reworked DUT passes Final Verification again.
    """
    emit("Authorizing Final Verification rework for this identity...")
    result = client.call("POST", "/api/rework/authorize", {
        "client_id": client._client_id(router),
        "mac": identity.get("mac", ""),
        "serial_number": identity.get("serial_number", ""),
        "gpon_number": identity.get("gpon_number", ""),
        "source": "MAC_REWRITE",
        "reason": (
            f"{router.name} {router.ip} via NIC {router.source_ip} | "
            "MAC Rewrite completed successfully; Final Verification must run again."
        ),
    })
    if result.get("authorization_required"):
        emit(
            "Final Verification rework authorization ACTIVE: "
            + str(result.get("authorization_id", ""))
        )
    else:
        emit("No prior Final Verification PASS exists; normal Final Verification is already allowed.")
    return result

def require_rewrite_admin_login() -> bool:
    """Require fixed admin credentials before the MAC Rewrite station opens."""
    accepted = {"ok": False}
    root = tk.Tk()
    root.title("MAC Rewrite - Admin Login")
    root.geometry("390x230")
    root.resizable(False, False)
    root.configure(bg=BRAND_BG)

    tk.Label(root, text="MAC REWRITE", font=("Segoe UI", 18, "bold"),
             bg=BRAND_BG, fg=BRAND_DARK).pack(pady=(18, 4))
    tk.Label(root, text="Authorized rework access only", font=("Segoe UI", 10),
             bg=BRAND_BG, fg=BRAND_MUTED).pack(pady=(0, 10))

    frame = tk.Frame(root, bg=BRAND_BG)
    frame.pack(fill="x", padx=34)

    tk.Label(frame, text="Username", bg=BRAND_BG, fg=BRAND_TEXT,
             anchor="w").pack(fill="x")
    user_var = tk.StringVar()
    user_entry = tk.Entry(frame, textvariable=user_var, font=("Segoe UI", 11))
    user_entry.pack(fill="x", pady=(2, 8))

    tk.Label(frame, text="Password", bg=BRAND_BG, fg=BRAND_TEXT,
             anchor="w").pack(fill="x")
    ...
    tk.Label(frame, textvariable=status, bg=BRAND_BG, fg="#B42318").pack(fill="x")

    def submit(event=None):
        if user_var.get().strip() == "admin" and pass_var.get() == "9278456590":
            accepted["ok"] = True
            root.destroy()
        else:
    ...
    btn = tk.Button(frame, text="LOGIN", command=submit, bg=BRAND_BLUE, fg="white",
...
class ClientApp(tk.Tk):
    ...
    def _scan_worker(self, router: RouterTarget, raw: str, cfg: dict[str, str]):
        try:
            ...
            # MAC Rewrite is a rework station for identities that have already
            # completed normal MAC Write.  A normal production identity is therefore
            # expected to be PASS here.  Never require AVAILABLE, because that would
            # incorrectly reject every previously programmed DUT.
            identity_state = str(identity.get("state", "")).upper().strip()
            if identity_state != "PASS":
                raise AppError(
                    f"MAC Rewrite requires an existing programmed identity in PASS state "
                    f"(current state={identity.get('state')})."
                )
            ...
            if require_pcb and not str(identity.get("pcb_serial_number", "")).strip():
                raise AppError("NO PCB SERIAL NUMBER PRESENT — Complete Box Build before MAC Rewrite.")
    ...
    def _cycle_worker(self, router: RouterTarget, client: ServerClient, request_id: str, cfg, write_cmds, verify_cmds, finalize_cmds, selected_identity=None):
        ...
        try:
            if not selected_identity:
                raise AppError("MAC Rewrite requires a scanned existing identity.")

            # This station is intentionally NON-BLOCKING:
            # it never allocates/reserves a MAC and never changes mac_pool PASS state.
            identity = dict(selected_identity)
            mac = identity.get("mac", "")
            serial = identity.get("serial_number", "")
            gpon = identity.get("gpon_number", "")
            if not mac or not serial or not gpon:
                raise AppError("Scanned identity is incomplete.")
            if str(identity.get("state", "")).upper() != "PASS":
                emit(f"WARNING: central identity state is {identity.get('state')}; rewrite will continue without reserving it.")

            emit(f"MAC REWRITE started for {mac} / {serial} / {gpon}")
            ...
            if firmware_enabled and not firmware.get("available"):
                raise AppError("Firmware update is ON but no firmware file is available.")

            if firmware_enabled:
                self.events.put(("firmware_start", (router.key, firmware)))
                emit(f"Starting firmware rewrite step: {firmware.get('file_name')} ({firmware.get('size', 0)} bytes)")
                local_fw = client.download_firmware(firmware)
                emit(f"Firmware downloaded and SHA-256 verified: {local_fw.name}")
                fw_result = run_direct_firmware_update(router, local_fw, cfg, emit)
                firmware_name = str(firmware.get("file_name") or "")
                emit("FIRMWARE UPDATE: PASS")
                ...
                emit("Firmware step skipped by configuration.")
                self.events.put(("firmware_skipped", (router.key,)))

            firstboot_pre = cfg.get("POST_FIRMWARE_FIRSTBOOT_COMMAND", "firstboot -y -r").strip()
            if firstboot_pre:
                emit("--- PRE-IDENTITY FIRSTBOOT ---")
                asyncio.run(v1370_telnet_commands(router, [firstboot_pre], emit, allow_disconnect_last=True))
                wait_for_router_after_firmware(
                    router,
            ...
            emit("--- CLEARALL + FACTORY MODE ---")
            asyncio.run(v1370_telnet_commands(
                router, [x for x in (clear_cmd, factory_cmd) if x], emit, strict=True
            ))

            self.events.put(("allocated", (router.key, {
                **identity,
                "reservation_id": "REWRITE-NONBLOCKING",
                "seq": 0,
            })))
            self.events.put(("telnet_start", (router.key,)))

            asyncio.run(v1370_write_identity(
                router, mac, serial, gpon, 0, write_cmds, emit
            ))

            firstboot_post = cfg.get("POST_WRITE_FIRSTBOOT_COMMAND", "firstboot -y -r").strip()
            if firstboot_post:
                emit("--- POST-WRITE FIRSTBOOT ---")
                asyncio.run(v1370_telnet_commands(router, [firstboot_post], emit, allow_disconnect_last=True))
                wait_for_router_after_firmware(
                    router,
            ...
            passed, checks = asyncio.run(v1370_verify_identity(
                router, mac, serial, gpon, 0, verify_cmds, emit
            ))
            ...
            if not passed:
                ...
                self.events.put(("done", (
                    router.key, mac, serial, gpon, "FAIL",
                    {"stats": {}, "rewrite": True, "final_verification_rework_authorized": False},
                )))
                return

            # PASS at MAC Rewrite must NOT lock/block the board.
            # Do not call normal MAC Rewrite /api/result and do not modify mac_pool.
            fv = _rewrite_mark_final_failed(client, router, identity, cfg, emit)
            set_pending_job(router.key, None)
            self.events.put(("done", (
                router.key, mac, serial, gpon, "PASS",
                {
                    "stats": {},
                    "rewrite": True,
                    "final_verification_rework_authorized": True,
                    "rework_authorization": fv,
            ...
            while True:
                ...
                if kind == "log":
                    ...
                    if status == "PASS":
                        ...
                        card.set_state("PASS", "MAC Rewrite PASS: firmware, firstboot, clearall, factorymode, identity write, second firstboot and post-reboot verification completed.", mac, serial, gpon)
                    elif status == "FAIL":
...
if __name__ == "__main__":
    if require_rewrite_admin_login():
        ClientApp().mainloop()
