...
APP_VERSION = "13.69-ETE-MAC-WRITER-FW-FIRST-FACTORYMODE-FIRSTBOOT"
...
async def post_firmware_board_prepare(router: RouterTarget, cfg: dict[str, str], emit) -> None:
    """Run factory mode + firstboot after firmware verification and before identity reservation/write."""
    ...
    firstboot_cmd = cfg.get("POST_FIRMWARE_FIRSTBOOT_COMMAND", "firstboot -y -r").strip()
    if not factory_cmd:
        raise AppError("POST_FIRMWARE_FACTORYMODE_COMMAND is blank.")
    if not firstboot_cmd:
        raise AppError("POST_FIRMWARE_FIRSTBOOT_COMMAND is blank.")

    emit("--- POST-FIRMWARE BOARD PREPARATION ---")
    emit(f"Connecting to {router.name} at {router.ip}:{router.port} via PC NIC {router.source_ip} ...")
    try:
        ...
        raise AppError(f"Could not connect for post-firmware preparation: {exc}") from exc
    ...
    try:
        ...
        failure_tokens = ("NOT FOUND", "UNKNOWN COMMAND", "INVALID COMMAND", "ERROR", "FAILED", "FAIL")
        if any(token in upper for token in failure_tokens):
            raise AppError(f"Factory mode command failed or was rejected: {response.strip()[:500]}")

        emit("--- FIRSTBOOT ---")
        emit(f"> {firstboot_cmd}")
        writer.write(firstboot_cmd + "\r\n")
        await asyncio.sleep(max(0.5, router.command_delay))
        try:
            response = await read_until(reader, router.command_prompt, min(3.0, router.timeout))
            if response:
                emit(response.rstrip())
        except Exception:
            emit("Telnet disconnected after firstboot (expected).")
    finally:
        try:
            writer.close()
        except Exception:
...
async def telnet_session(router: RouterTarget, mac: str, serial: str, gpon: str, seq: int,
    ...
    try:
        ...
        # Optional pre-write command. v13.69 leaves this blank by default because
        # factorymode + firstboot have already prepared the board.
        pre_write_command = parse_key_value_file(CONFIG_PATH).get("PRE_WRITE_COMMAND", "").strip()
        if pre_write_command:
            emit("--- OPTIONAL PRE-WRITE COMMAND ---")
...
class ClientApp(tk.Tk):
    ...
    def _cycle_worker(self, router: RouterTarget, client: ServerClient, request_id: str, cfg, write_cmds, verify_cmds, finalize_cmds, selected_identity=None):
        ...
        try:
            ...
            if firmware_enabled:
                ...
                emit(f"STEP 1/5 — FIRMWARE FIRST: {firmware_name} ({firmware.get('size', 0)} bytes)")
                ...
                if after_text:
                    emit(f"Verified post-upgrade board info: {after_text[:500]}")
                self.events.put(("firmware_done", (router.key, firmware)))
            else:
                emit("Firmware update OFF on central server — firmware step skipped.")
            ...
            emit("STEP 2/5 — FACTORY MODE, then FIRSTBOOT.")
            asyncio.run(post_firmware_board_prepare(router, cfg, emit))

            wait_for_router_after_firmware(
                router,
                float(cfg.get("POST_FIRSTBOOT_INITIAL_DELAY_SECONDS", "10")),
                float(cfg.get("POST_FIRSTBOOT_TIMEOUT_SECONDS", "180")),
                float(cfg.get("POST_FIRSTBOOT_POLL_SECONDS", "2")),
                emit,
            )
            ...
            emit("STEP 3/5 — Board prepared. Reserving identity from central server now.")
            try:
                if selected_identity:
                ...
                if "NO PCB SERIAL NUMBER PRESENT" in str(exc).upper():
                    set_pending_job(router.key, None)
                    self.events.put(("pcb_missing", (router.key, "NO PCB SERIAL NUMBER PRESENT — identity was not written.")))
                    return
                raise
            ...
            set_pending_job(router.key, {
                ...
                "gpon_number": gpon,
                "seq": seq,
                "state": "RESERVED_AFTER_BOARD_PREP",
                "created_at": time.time(),
            })
            self.events.put(("allocated", (router.key, alloc)))
            self.events.put(("telnet_start", (router.key,)))

            emit("STEP 4/5 — Writing MAC / Serial / GPON.")
            passed, checks = asyncio.run(
                telnet_session(router, mac, serial, gpon, seq, write_cmds, verify_cmds, [], emit)
            )
            if firmware_enabled:
                checks["FIRMWARE"] = True
            checks["FACTORYMODE"] = True
            checks["FIRSTBOOT"] = True
            self.events.put(("verify_result", (router.key, passed, checks, firmware_enabled)))

            if not passed:
                status = "FAIL"
                detail = f"Identifier verification failed after firmware/factory preparation: {checks}"
            else:
                status = "PASS"
                detail = (
                    f"Firmware first: {firmware_name or 'skipped'} | "
                    "factorymode=1 | firstboot complete | MAC, serial and GPON verified"
                )

            emit("STEP 5/5 — Reporting MAC Write result to central server.")
    ...
    def _resume_worker(self, router, client, pending, cfg, write_cmds, verify_cmds, finalize_cmds):
        ...
        try:
            emit("v13.69 recovery: board preparation completed before this identity was reserved.")
            emit("Writing/verifying the reserved identity without reflashing firmware or rerunning firstboot.")
            passed, checks = asyncio.run(
                telnet_session(router, mac, serial, gpon, seq, write_cmds, verify_cmds, [], emit)
            )
            checks["BOARD_PREP_RECOVERY"] = True
            self.events.put(("verify_result", (router.key, passed, checks, False)))
            detail = (
                "Recovered v13.69 reserved identity after board preparation | identifiers verified"
                if passed else f"Recovered v13.69 reserved identity | verification failed: {checks}"
            )
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    card.set_step("reserve", "✓")
                    card.set_step("write", "…")
                    card.set_state("PROGRAMMING", "Identity reserved after firmware/factory preparation; writing MAC, serial and GPON over Telnet",
